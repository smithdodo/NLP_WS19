...
def train(df):
    """
    Return p(w|c) and p(c)
    """
    # p(c)
    pc = {k: v/len(df) for k,v in Counter(df['class_name']).items()}

    # convert word counts to relative freq
    df['length'] = df.iloc[:, 1:-1].sum(axis=1)
    df = df.groupby('class_name').sum()
    # p(w|c)
    df= df.iloc[:, 0:-1].div(df['length'], axis=0)
...
def predict(word_counts, p_w_c, prior):
    ...
    for class_, row in p_w_c.iterrows():
        val = prior[class_] * np.prod([row[word] ** count for word, count in word_counts.items()])
        if val > best_val:
            best_class = class_
            best_val = val
    
    print(best_class, best_val)
    return best_class


def evaluate():
    voc = get_voc(400, '20news/20news.voc')
    ...
    for idx, row in test.iterrows():
        ...
        pred = predict(row[1:], p_w_c, prior)
